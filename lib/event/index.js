...
function Event(name, history) {
    var _subscriptions = keyed(function (item) {
    ...
    var _publishHistorical = function (subscription) {
        var historical = history.get(name);
    ...
    this.add = function (listener, priority, publicationStrategy, acceptHistorical) {
        var subscription = new Subscription(new SubscriptionToken(this), listener, publicationStrategy, priority);
    ...
    this.remove = function (token) {
        var subscription = _subscriptions.get(token.getUid());
        if (subscription) {
...
function SubscriptionBuilder(event) {
    ...
    var _pubStrategy = new SyncPublicationStrategy();
    var _priority = Priority.MEDIUM;
    var _acceptHistorical = false;
    var _tokens = undefined;
    ...
    this.add = function (listener) {
        var token = event.add(listener, _priority, _pubStrategy, _acceptHistorical);
        if (_tokens) {
            _tokens.add(token);
        }
        return token;
...
function EventAggregator(historicalLimit) {
    ...
    var _history = new EventHistory(historicalLimit);

    var _events = keyed(function (event) {
        return event.getName()
    }).collection();

    this.get = function (name) {
        var event = _events.getOrCreate(name, function () {
